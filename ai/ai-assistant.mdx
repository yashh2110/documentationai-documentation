...
Where supported, you can also customize the assistant on a per-document basis. This lets you tailor the experience for a specific page or section with settings such as a custom fallback channel, custom instructions, response language, starter questions, and follow-up behavior.
...
On documents where per-document customization is configured, users may also see starter questions, receive responses in a chosen language, or be guided to a custom fallback channel when the answer should continue somewhere else.

Those document-level settings can also shape how the assistant follows up, so the experience feels more relevant to the page the user is viewing.
...
- **Tailor the experience by document** – apply page-specific instructions, language preferences, starter questions, fallback guidance, and follow-up behavior where a more opinionated experience is helpful.
