...
title: Code Editor
description: >-
  Use Git-based workflows and MDX in your preferred code editor to write,
  organize, and publish documentation with Documentation.AI.
createdAt: '2025-12-29T11:44:28.465Z'
updatedAt: '2025-12-29T11:44:28.465Z'
---

## Overview

The code editor workflow lets you write and manage documentation in your preferred development environment (for example, VS Code, Cursor, or Windsurf). You work with `.mdx` files locally, commit changes to Git, choose a deployment branch for production, and use other branches for preview deployments and pull request review.
...
4. Choose the branch that should deploy to production (typically `main` or `master`).

After the connection, pushes to the configured deployment branch trigger a production build and deployment. Pushes to other branches can create preview deployments so you can review changes before they go live.
...
4. Use that branch for a preview deployment and open a pull request (PR) when you are ready for review.
5. Merge into your chosen deployment branch to publish to production.
...
<Steps>
  ...
  <Step title="Push your branch and share a preview" icon="upload">

  ```bash
  git push origin add-webhook-docs
  ```

  Documentation.AI can create a preview deployment from that non-deployment branch so you can review rendered changes before publishing. When you want feedback or approval, open a pull request in your Git provider and share the preview with reviewers.

  </Step>

  <Step title="Review and merge" icon="check-circle">

  - Use the preview deployment to verify formatting, navigation, and links.
  - Address review comments and push updates to the same branch.
  - When ready, merge the PR into the deployment branch you selected in Documentation.AI.

  Documentation.AI then runs a production build from that deployment branch and publishes the changes.
...
Use feature branches and PRs even for documentation-only changes. Documentation.AI can generate preview deployments from non-deployment branches, which helps reviewers catch structural or navigation issues before those changes reach your production deployment branch.
...
- **Deployment did not update**  
  - Confirm that your changes were merged into the configured deployment branch. A preview deployment from another branch does not update production.  
  - Verify that the latest commit on the deployment branch triggered a build in your Documentation.AI project.
...
- Use branches, commits, and PRs to collaborate, with preview deployments for non-deployment branches.
- Let Documentation.AI publish from the deployment branch you choose for production.
