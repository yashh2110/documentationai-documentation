...
type: reference
createdAt: '2026-04-10T07:33:08.374Z'
updatedAt: '2026-04-10T07:33:08.374Z'
---
## Overview
...
Control which integrations and scripts require user consent before loading. Define consent categories under `integrations.cookies`, then assign categories to your integrations and scripts. Built-in telemetry follows its own `integrations.telemetry` settings and can also be gated when cookie consent is active. See the [Cookie Consent guide](/customize/cookie-consent) for step-by-step setup and CMP examples.
...
Control Documentation.ai's built-in page view tracking. Disable it entirely with `enabled: "false"`, or keep it enabled and gate it behind a consent category when cookie consent is active.
...
<ParamField path="integrations.telemetry.enabled" param-type="string" required="false" deprecated="false">
  ...
  Set to `"false"` to disable built-in page view tracking entirely. Defaults to enabled.

</ParamField>

<ParamField path="integrations.telemetry.category" param-type="string" required="false" deprecated="false">

  Consent category for telemetry when cookie consent is active and telemetry remains enabled. Defaults to `"analytics"`.
